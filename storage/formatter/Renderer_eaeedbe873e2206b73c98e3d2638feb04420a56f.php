...
*/
class Renderer_eaeedbe873e2206b73c98e3d2638feb04420a56f extends \s9e\TextFormatter\Renderers\PHP
{
	protected $params=['DISCUSSION_URL'=>'http://localhost/flarum17/public/d/','L_WROTE'=>'','PROFILE_URL'=>'http://localhost/flarum17/public/u/','TAG_URL'=>'http://localhost/flarum17/public/t/'];
    ...
    </script></figure>';break;case'URL':$this->out.='<a href="'.htmlspecialchars($node->getAttribute('url'),2).'"';if($node->hasAttribute('rel'))$this->out.=' rel="'.htmlspecialchars($node->getAttribute('rel'),2).'"';if($node->hasAttribute('target'))$this->out.=' target="'.htmlspecialchars($node->getAttribute('target'),2).'"';if($node->hasAttribute('title'))$this->out.=' title="'.htmlspecialchars($node->getAttribute('title'),2).'"';$this->out.='>';$this->at($node);$this->out.='</a>';break;case'USERMENTION':if($node->getAttribute('deleted')!=1)$this->out.='<a href="'.htmlspecialchars($this->params['PROFILE_URL'].$node->getAttribute('slug'),2).'" class="UserMention">@'.htmlspecialchars($node->getAttribute('displayname'),0).'</a>';else$this->out.='<span class="UserMention UserMention--deleted">@'.htmlspecialchars($node->getAttribute('displayname'),0).'</span>';break;case'br':$this->out.='<br>';break;case'e':case'i':case's':break;case'html:iframe':$this->out.='<iframe';if($node->hasAttribute('style'))$this->out.=' style="'.htmlspecialchars($node->getAttribute('style'),2).'"';if($node->hasAttribute('src'))$this->out.=' src="'.htmlspecialchars($node->getAttribute('src'),2).'"';if($node->hasAttribute('scrolling'))$this->out.=' scrolling="'.htmlspecialchars($node->getAttribute('scrolling'),2).'"';if($node->hasAttribute('allow'))$this->out.=' allow="'.htmlspecialchars($node->getAttribute('allow'),2).'"';if($node->hasAttribute('allowpaymentrequest'))$this->out.=' allowpaymentrequest="'.htmlspecialchars($node->getAttribute('allowpaymentrequest'),2).'"';if($node->hasAttribute('credentialless'))$this->out.=' credentialless="'.htmlspecialchars($node->getAttribute('credentialless'),2).'"';if($node->hasAttribute('csp'))$this->out.=' csp="'.htmlspecialchars($node->getAttribute('csp'),2).'"';if($node->hasAttribute('height'))$this->out.=' height="'.htmlspecialchars($node->getAttribute('height'),2).'"';if($node->hasAttribute('loading'))$this->out.=' loading="'.htmlspecialchars($node->getAttribute('loading'),2).'"';if($node->hasAttribute('name'))$this->out.=' name="'.htmlspecialchars($node->getAttribute('name'),2).'"';if($node->hasAttribute('referrerpolicy'))$this->out.=' referrerpolicy="'.htmlspecialchars($node->getAttribute('referrerpolicy'),2).'"';if($node->hasAttribute('sandbox'))$this->out.=' sandbox="'.htmlspecialchars($node->getAttribute('sandbox'),2).'"';if($node->hasAttribute('width'))$this->out.=' width="'.htmlspecialchars($node->getAttribute('width'),2).'"';if($node->hasAttribute('align'))$this->out.=' align="'.htmlspecialchars($node->getAttribute('align'),2).'"';if($node->hasAttribute('frameborder'))$this->out.=' frameborder="'.htmlspecialchars($node->getAttribute('frameborder'),2).'"';if($node->hasAttribute('longdesc'))$this->out.=' longdesc="'.htmlspecialchars($node->getAttribute('longdesc'),2).'"';if($node->hasAttribute('marginheight'))$this->out.=' marginheight="'.htmlspecialchars($node->getAttribute('marginheight'),2).'"';if($node->hasAttribute('marginwidth'))$this->out.=' marginwidth="'.htmlspecialchars($node->getAttribute('marginwidth'),2).'"';$this->out.='>';$this->at($node);$this->out.='</iframe>';break;case'p':$this->out.='<p>';$this->at($node);$this->out.='</p>';break;default:$this->at($node);}
	}
	/** {@inheritdoc} */
	...
	protected $static=['/B'=>'</b>','/C'=>'</code>','/CENTER'=>'</div>','/CODE'=>'</code><script async="" crossorigin="anonymous" data-hljs-style="github" integrity="sha384-E9ssooeJ4kPel3JD7st0BgS50OLWFEdg4ZOp8lYPy52ctQazOIV37TCvzV8l4cYG" src="https://cdn.jsdelivr.net/gh/s9e/hljs-loader@1.0.34/loader.min.js"></script><script>
                ...
                </script></pre>','/COLOR'=>'</span>','/DEL'=>'</del>','/EM'=>'</em>','/EMAIL'=>'</a>','/ESC'=>'','/H1'=>'</h1>','/H2'=>'</h2>','/H3'=>'</h3>','/H4'=>'</h4>','/H5'=>'</h5>','/H6'=>'</h6>','/I'=>'</i>','/ISPOILER'=>'</span>','/LI'=>'</li>','/QUOTE'=>'</div></blockquote>','/S'=>'</s>','/SIZE'=>'</span>','/SPOILER'=>'</details>','/STRONG'=>'</strong>','/SUB'=>'</sub>','/SUP'=>'</sup>','/TABLE'=>'</table>','/TBODY'=>'</tbody>','/TD'=>'</td>','/TH'=>'</th>','/THEAD'=>'</thead>','/TR'=>'</tr>','/U'=>'</u>','/URL'=>'</a>','/html:iframe'=>'</iframe>','B'=>'<b>','C'=>'<code>','CENTER'=>'<div style="text-align:center">','DEL'=>'<del>','EM'=>'<em>','ESC'=>'','H1'=>'<h1>','H2'=>'<h2>','H3'=>'<h3>','H4'=>'<h4>','H5'=>'<h5>','H6'=>'<h6>','HR'=>'<hr>','I'=>'<i>','ISPOILER'=>'<span class="spoiler" onclick="removeAttribute(\'class\')">','LI'=>'<li>','S'=>'<s>','SPOILER'=>'<details class="spoiler">','STRONG'=>'<strong>','SUB'=>'<sub>','SUP'=>'<sup>','TABLE'=>'<table>','TBODY'=>'<tbody>','THEAD'=>'<thead>','TR'=>'<tr>','U'=>'<u>'];
	/** {@inheritdoc} */
	protected $dynamic=['COLOR'=>['(^[^ ]+(?> (?!color=)[^=]+="[^"]*")*(?> color="([^"]*)")?.*)s','<span style="color:$1">'],'EMAIL'=>['(^[^ ]+(?> (?!email=)[^=]+="[^"]*")*(?> email="([^"]*)")?.*)s','<a href="mailto:$1">'],'IMG'=>['(^[^ ]+(?> (?!(?:alt|height|src|title|width)=)[^=]+="[^"]*")*(?> alt="([^"]*)")?(?> (?!(?:height|src|title|width)=)[^=]+="[^"]*")*( height="[^"]*")?(?> (?!(?:src|title|width)=)[^=]+="[^"]*")*(?> src="([^"]*)")?(?> (?!(?:title|width)=)[^=]+="[^"]*")*(?> title="([^"]*)")?(?> (?!width=)[^=]+="[^"]*")*( width="[^"]*")?.*)s','<img src="$3" title="$4" alt="$1"$2$5>'],'SIZE'=>['(^[^ ]+(?> (?!size=)[^=]+="[^"]*")*(?> size="([^"]*)")?.*)s','<span style="font-size:$1px">'],'UPL-IMAGE-PREVIEW'=>['(^[^ ]+(?> (?!(?:alt|title|u(?:rl|uid))=)[^=]+="[^"]*")*(?> alt="([^"]*)")?(?> (?!(?:title|u(?:rl|uid))=)[^=]+="[^"]*")*(?> title="([^"]*)")?(?> (?!u(?:rl|uid)=)[^=]+="[^"]*")*(?> url="([^"]*)")?(?> (?!uuid=)[^=]+="[^"]*")*(?> uuid="([^"]*)")?.*)s','<img class="FoFUpload--Upl-Image-Preview" src="$3" title="$2" alt="$1" data-id="$4" loading="lazy">'],'URL'=>['(^[^ ]+(?> (?!(?:rel|t(?:arget|itle)|url)=)[^=]+="[^"]*")*( rel="[^"]*")?(?> (?!(?:t(?:arget|itle)|url)=)[^=]+="[^"]*")*( target="[^"]*")?(?> (?!(?:title|url)=)[^=]+="[^"]*")*( title="[^"]*")?(?> (?!url=)[^=]+="[^"]*")*(?> url="([^"]*)")?.*)s','<a href="$4"$1$2$3>'],'html:iframe'=>['(^[^ ]+(?> (?!(?:al(?:ign|low(?:paymentrequest)?)|c(?:redentialless|sp)|frameborder|height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( align="[^"]*")?(?> (?!(?:allow(?:paymentrequest)?|c(?:redentialless|sp)|frameborder|height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( allow="[^"]*")?(?> (?!(?:allowpaymentrequest|c(?:redentialless|sp)|frameborder|height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( allowpaymentrequest="[^"]*")?(?> (?!(?:c(?:redentialless|sp)|frameborder|height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( credentialless="[^"]*")?(?> (?!(?:csp|frameborder|height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( csp="[^"]*")?(?> (?!(?:frameborder|height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( frameborder="[^"]*")?(?> (?!(?:height|lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( height="[^"]*")?(?> (?!(?:lo(?:ading|ngdesc)|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( loading="[^"]*")?(?> (?!(?:longdesc|margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( longdesc="[^"]*")?(?> (?!(?:margin(?:height|width)|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( marginheight="[^"]*")?(?> (?!(?:marginwidth|name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( marginwidth="[^"]*")?(?> (?!(?:name|referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( name="[^"]*")?(?> (?!(?:referrerpolicy|s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( referrerpolicy="[^"]*")?(?> (?!(?:s(?:andbox|crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( sandbox="[^"]*")?(?> (?!(?:s(?:crolling|rc|tyle)|width)=)[^=]+="[^"]*")*( scrolling="[^"]*")?(?> (?!(?:s(?:rc|tyle)|width)=)[^=]+="[^"]*")*( src="[^"]*")?(?> (?!(?:style|width)=)[^=]+="[^"]*")*( style="[^"]*")?(?> (?!width=)[^=]+="[^"]*")*( width="[^"]*")?.*)s','<iframe$17$16$15$2$3$4$5$7$8$12$13$14$18$1$6$9$10$11>']];
	/** {@inheritdoc} */
